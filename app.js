...
app.get('/auth/google/callback',passport.authenticate('google', { failureRedirect: '/error' }),function(req, res){
    ...
    //SELECT Usu_id,Usu_contraseña,Usu_nombre FROM cellgadb.usuario WHERE Usu_id='nicolas@gmail.com';
    conexion.query("SELECT Usu_id,Usu_contraseña,Usu_nombre FROM cellgadb.usuario WHERE Usu_id='"+email+"';",(err,resp)=>{
      ...
      if(resp.length <= 0){
        ...
        conexion.query('INSERT INTO cellgadb.tabla_individual(Ti_nombre,Usu_id) VALUES("tabla de '+displayName+'", "'+email+'" );',(error3,results3)=>{
          if(error3) throw error3;
          console.log('Tabla individual creada',results3);
          
        });

        conexion.query("SELECT Usu_id,Usu_contraseña,Usu_nombre FROM cellgadb.usuario WHERE Usu_id='"+email+"';",(err5,resp5)=>{
          if(err5) throw err5
          conexion.query('SELECT Ti_id FROM cellgadb.tabla_individual WHERE Usu_id = "'+resp5[0].Usu_id+'";',(err6,results6)=>{
            if(err6) throw err6;
...
app.get('/indexequipo',(req,res)=>{
  ...
  if(req.session.user){
    ...
    conexion.query("SELECT Au_id, Ro_id, Te_id, Te_nombre, Te_descripcion FROM cellgadb.accesousu WHERE Usu_id ='"+ usuario[0].Usu_id +"';",(err,resp)=>{
      if(err) throw err;
...
app.get('/subtablero/:Te_id',(req,res)=>{
  ...
  conexion.query("SELECT Ro_id FROM cellgadb.accesousu WHERE Te_id = '"+te_id+"' AND Usu_id = '"+usuario[0].Usu_id+"';",(err1,resp1)=>{
    if(err1) throw err1;
    if(resp1[0].Ro_id === 1){
      conexion.query("SELECT Tare_id, Tare_tema, Tare_descripcion, Tare_fechaExp, Tare_color FROM cellgadb.tarea_te WHERE Te_id= '"+te_id+"';",(error,results)=>{
        if(error) throw error;
        conexion.query("SELECT Te_tema, Te_ca FROM cellgadb.tabla_equipo WHERE Te_id='"+te_id+"';",(err1,resp1)=>{
          if(err1) throw err1
      ...
      conexion.query("SELECT Tare_id, Tare_tema, Tare_descripcion, Tare_fechaExp, Tare_color FROM cellgadb.tarea_te WHERE Te_id= '"+te_id+"';",(error,results)=>{
        if(error) throw error;
        conexion.query("SELECT Te_tema, Te_ca FROM cellgadb.tabla_equipo WHERE Te_id='"+te_id+"';",(err2,resp2)=>{
          if(err2) throw err2
...
app.get('/eliminarTe/:id',function(req,res){
  const id= req.params.id;
  conexion.query('DELETE FROM cellgadb.tarea_te WHERE Tare_id=?',[id],(error,results)=>{
    if(error)throw error;
    res.redirect('/index');
  })
});
...
app.get('/actualizarTe/:id',function(req,res){
  const id=req.params.id;
  conexion.query("SELECT * FROM cellgadb.tarea_te WHERE Tare_id=?",[id],(error,results)=>{
    if(error)throw error;
...
app.post("/actualizarTeDB",function(req,res){
  ...
  var color = req.body.color;
  console.log(color);
    conexion.query("UPDATE cellgadb.tarea_te SET Tare_color= '"+color+"', Tare_tema = '"+Tema+"', Tare_descripcion = '"+Descripcion+"', Tare_fechaCrea = CURRENT_TIMESTAMP(), Tare_fechaExp='"+fechaFin+"' WHERE Tare_id='"+id+"';",function(error,results){
      if(error) throw error;
      conexion.query('SELECT Te_id FROM cellgadb.tarea_te  WHERE Tare_id = "'+id+'";',(error2,results2)=>{
        if(error2)throw error2
...
app.post('/agregarActividadTEDB/:te_id',(req,res)=>{
  ...
  console.log("color",color);
  console.log('antes del query de insertar');
  conexion.query("INSERT INTO cellgadb.tarea_te(Tare_tema,Tare_descripcion,Tare_fechaCrea,Tare_fechaExp,Tare_color,Te_id) VALUES ('"+temaForm+"','"+descripcionForm+"',CURRENT_TIMESTAMP,'"+FechaTerminoForm+"','"+color+"','"+Te_id+"');",(err,results2)=>{
    if(err) throw err;
    console.log('Se agrego actividad',results2);
  });
  console.log('despues del query de agregar')
...
app.post('/CrearTableroEquipo',function(req,res){
  ...
  conexion.query("SELECT Te_id FROM cellgadb.tabla_equipo WHERE Te_id ='"+idin+"';",function(err,result){
    if(err)throw err;
    conexion.query("SELECT Te_ca FROM cellgadb.tabla_equipo WHERE Te_ca='"+ codeaccess +"';",function(err2,result2){
      if(err2)throw err2;
      ...
      if(idout === 1 || codeaccessr === 1){
        ...
        conexion.query("INSERT INTO cellgadb.tabla_equipo(Te_id,Te_tema,Te_descripcion,Te_ca) VALUES('"+idin+"','"+nombretablero+"','"+descripciontablero+"','"+codeaccess+"');",function(err2,resp2){
          if(err2)throw err2;
        });
        conexion.query("INSERT INTO cellgadb.accesousu(Ro_id,Usu_id,Te_id,Te_nombre,Te_descripcion) VALUES('1','"+user+"','"+idin+"','"+nombretablero+"','"+descripciontablero+"');",function(err1,resp1){
          if(err1)throw err1;
        });
      }
...
app.post('/unirEquipo',function(req,res){
  ...
  var user = req.session.user[0].Usu_id;
  conexion.query("SELECT Te_id, Te_tema, Te_descripcion FROM cellgadb.tabla_equipo WHERE Te_ca ='"+idre+"';",function(err,resp){
    if(err) throw err;
    var respp = resp;
    if(respp.length > 0){
      conexion.query("SELECT * FROM cellgadb.accesousu WHERE Usu_id='"+user+"' AND Te_id = '"+resp[0].Te_id+"';",(err3,resp3)=>{
        if(err3) throw err3;
        if(resp3.length > 0){
          res.redirect('/indexequipo')
        }else{
          console.log("La tabla si existe se integrara en breve")
          conexion.query("INSERT INTO cellgadb.accesousu(Ro_id,Usu_id,Te_id,Te_nombre,Te_descripcion) VALUES('2','"+user+"','"+resp[0].Te_id+"','"+resp[0].Te_tema+"','"+resp[0].Te_descripcion+"');",function(err2,resp2){
            if (err2) throw err2;
            res.redirect('/indexequipo');
          });
        }
...
app.get('/salirEquipo/:Te_id',function(req,res){
  ...
  conexion.query("DELETE FROM cellgadb.accesousu WHERE Usu_id='"+usuario+"' AND Te_id ='"+te_id+"';",(err,resp)=>{
    if(err) throw err;
